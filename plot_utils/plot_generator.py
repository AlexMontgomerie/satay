...
plot_generation = "yolo_comparison"
...
if plot_generation in ["yolo_comparison", "all"]:
    cpu_gpu_df = pd.read_csv('Results_CPU_and_GPU_and_FPGA.csv')
    ...
    # Version 2: Plotting the results for the different models on different input sizes
        ...
        # filter based on Platform
        cpu_gpu_df = cpu_gpu_df[cpu_gpu_df['Platform'].isin(['ARM Cortex-A72', 'Jetson TX2', 'VCU118'])]
        # TODO: Add in the list above the FPGA filtering
        cpu_gpu_df = cpu_gpu_df[cpu_gpu_df['Platform'].isin(['ARM Cortex-A72', 'Jetson TX2', 'VCU118'])]
        print(cpu_gpu_df['Platform'])
        fig, ax = plt.subplots()
        ...
        ax.set_xlabel('Inference Time (ms)', fontsize=FONT_SIZE_LABELS)
        ax.set_ylabel('mAP50-95', fontsize=FONT_SIZE_LABELS)
        ax.set_xscale('log')
        handles, labels = ax.get_legend_handles_labels()
...
if plot_generation in ["energy_comparison", "all"]:
    ...
    if energy_comparison_plot_type == 'bar':
        ...
        ax.set_ylabel('Inference Energy (mJ)', fontsize=FONT_SIZE_LABELS)
...
if plot_generation in ["ablation", "all"]:
    ...
    ax.set_xlabel('\# Buffers Removed', fontsize=FONT_SIZE_LABELS)
...
if plot_generation in ["pareto", "all"]:
    ...
    pareto_df = pd.read_csv('Pareto_Evaluation_new.csv').set_index('Work').T
    ...
    for idx in range(len(works)):
        if "Ours" in works[idx] and '.' in works[idx]:
            works[idx] = works[idx].split('.')[0]
        if "Pestana" in works[idx]:
            works[idx] = "D. Pestana (2021)"
    ...
    # pareto_df.drop(pareto_df.loc[12:-1].index, inplace=True)
    pareto_df = pareto_df.head(12)
    print(pareto_df)
    ...
    # for line in range(0, 12):
        ...
        if 'Pestana' in work_name:
            ax.text(curr_series['GOP/s/DSP'], curr_series['mAP50 (\%)'], work_name, ha='left', va='center', size=FONT_SIZE_TEXT, color='black', weight='bold')
    ...
    ax.set_xlabel('Performance Density (GOP/s/DSP)', fontsize=FONT_SIZE_LABELS)
    ax.set_ylabel('mAP @ 50 (\%)', fontsize=FONT_SIZE_LABELS)

    ax.set_ylim(30, 70)
    ax.set_xlim(-0.02, 0.275)
