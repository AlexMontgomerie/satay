...
conv_l.inputs[1] = gs.Constant("model.24.m.2.weight", np.concatenate((conv_l.inputs[1].values, np.zeros((1, conv_l_channels, 1, 1), dtype=np.float16))))
conv_l.inputs[2] = gs.Constant("model.24.m.2.bias", np.concatenate((conv_l.inputs[2].values, np.zeros((1), dtype=np.float16))))

conv_m_channels = conv_m.inputs[1].values.shape[1]
conv_m.inputs[1] = gs.Constant("model.24.m.1.weight", np.concatenate((conv_m.inputs[1].values, np.zeros((1, conv_m_channels, 1, 1), dtype=np.float16))))
conv_m.inputs[2] = gs.Constant("model.24.m.1.bias", np.concatenate((conv_m.inputs[2].values, np.zeros((1), dtype=np.float16))))

conv_r_channels = conv_r.inputs[1].values.shape[1]
conv_r.inputs[1] = gs.Constant("model.24.m.0.weight", np.concatenate((conv_r.inputs[1].values, np.zeros((1, conv_r_channels, 1, 1), dtype=np.float16))))
conv_r.inputs[2] = gs.Constant("model.24.m.0.bias", np.concatenate((conv_r.inputs[2].values, np.zeros((1), dtype=np.float16))))

# create the output nodes
output_l = gs.Variable("/model.24/m.0/Conv_output_0", shape=[1, 256, input_shape//32, input_shape//32], dtype="float16")
output_m = gs.Variable("/model.24/m.1/Conv_output_0", shape=[1, 256, input_shape//16, input_shape//16], dtype="float16")
output_r = gs.Variable("/model.24/m.2/Conv_output_0", shape=[1, 256, input_shape//8,  input_shape//8], dtype="float16")
